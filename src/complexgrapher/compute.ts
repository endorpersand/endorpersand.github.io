...
function computeBuffer(ev: Evaluator, cd: CanvasData): ArrayBuffer {
    let {f, inverse} = ev;
    let {width, height} = cd;
    ...
    for (var i = 0; i < width; i++) {
        for (var j = 0; j < height; j++) {
            let k = width * j + i;
            // compute value
            let fz = forceComplex(f( convPlanes(i, j, cd) ));
...
function convPlanes(x: number, y: number, cd: CanvasData) {
